    ...
    module Reminders
      ...
      def reminders_stage(options={})
        ...
        begin
          ...
          run_cmd %Q{rsync -ave ssh --delete --force --exclude='/storage/' #{reminders_git_checkout_path} --exclude='/vendor/' --exclude='.git/' --exclude='.gitignore' --filter "protect .env" --filter "protect down" --filter "protect reminders/vendor/" --filter "protect reminders/storage/*" --filter "protect reminders/storage/framework/*" #{reminders_user}@#{reminders_stage_ip}:#{site_root}}
