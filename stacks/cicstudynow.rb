    ...
    module Cicstudynow
      ...
      def cicstudynow_dev(options={})
        ...
        begin
          ...
          run_cmd %Q{rsync -lptgoDv --dirs --delete --force --exclude='.gitignore' #{cicstudynow_git_checkout_path}/app/storage/ #{site_path}/app/storage}
          ...
          # run db migrations
          #run_cmd %Q{cd #{site_path} && /usr/bin/php artisan migrate} # NOTE no db migrations for this site
      ...
      def cicstudynow_prod(options={})
        ...
        begin
          # take application offline (maintenance mode)
          run_cmd %Q{ssh #{cicstudynow_prod_user}@#{cicstudynow_prod_ip} "cd #{site_path} && /usr/bin/php artisan down || true"} # return true so command is non-fatal (artisan doesn't exist the first time)

          # TODO figure out how to keep from deleting cicstudynow/app/storage/meta/down (which enables the site)
          ...
          #run_cmd %Q{ssh #{cicstudynow_prod_user}@#{cicstudynow_prod_ip} "cd #{site_path} && /usr/bin/php artisan migrate --force"} # NOTE no db migrations for this site
